...
tiempox8va = [2.44, 1.87, 1.88, 2.14, 2.52, 2.55, 2.16, 1.61, 0.88, 0.65]
...
def calcular_parametros_avanzados(opciones):
    ...
    def procesar_banda(senal_filtrada, nombre_banda):
        banda_resultados = {}
        sch = None  # aseguramos definición
        reg = {}    # aseguramos tipo válido
        ...
        if opciones["hacer_schroeder"].get():
            try:
                ...
                if opciones["hacer_regresion"].get():
                    ...
                    banda_resultados["Regresión"] = reg
        ...
        # --- Parámetros ---
        reg = banda_resultados.get("Regresión", reg)
        if not isinstance(reg, dict):
            print(f"Advertencia: El campo 'Regresión' en {nombre_banda} no es un diccionario. Valor recibido: {reg}")
            reg = {}
        ...
        if nombre_banda == "General" and opciones.get("graficar", tk.BooleanVar(value=False)).get():
            ...
            if isinstance(reg, dict) and "tiempo" in reg:
...
titulo = tk.Label(root, text="Software para el cálculo de parámetros acústicos ISO 3382", font=("Arial", 16))
titulo.pack(pady=20)
